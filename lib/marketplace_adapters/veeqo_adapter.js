...
export async function fetchVeeqoOrders_adapted(userId, veeqoApiKey, prisma) {
  ...
  try {
    ...
    for (const vOrder of veeqoOrders) {
      ...
      const customerFullName = [customerFirstName, customerLastName].filter(Boolean).join(' ').trim() || null;
      ...
      const orderStatus = get(vOrder, 'status.name', 'unknown').toUpperCase().replace(/ /g, '_'); // e.g., AWAITING_FULFILLMENT

      const shippingAddress = {
        name: [get(vOrder, 'deliver_to.first_name'), get(vOrder, 'deliver_to.last_name')].filter(Boolean).join(' ').trim(),
        company: get(vOrder, 'deliver_to.company'),
        street1: get(vOrder, 'deliver_to.address1'),
        street2: get(vOrder, 'deliver_to.address2'),
        ...
        phone: get(vOrder, 'deliver_to.phone')
      };
      ...
      const orderDataToUpsert = {
        ...
        status: orderStatus,
        shipByDate: shipBy,
        currency: get(vOrder, 'currency_code'),
        totalPrice: parseFloat(get(vOrder, 'total_including_tax', 0)),
        shippingAddress: shippingAddress,
        // billingAddress: billingAddressData, // if available
      };
      ...
      if (Array.isArray(vOrder.line_items)) {
        for (const vItem of vOrder.line_items) {
          ...
          const lineImageUrl = get(vItem, 'image_url') || get(vItem, 'sellable.image_url') || get(vItem, 'sellable.images.0.url');
          
          const lineOpt = typeof get(vItem, 'additional_options') === 'string' ? get(vItem, 'additional_options', '').trim() : '';
          const combinedNotes = [lineOpt, orderNoteText].filter(Boolean).join(' | ');
          ...
          lineItemsData.push({
            ...
            totalPrice: lineQuantity * lineUnitPrice, // Calculate based on unit price
