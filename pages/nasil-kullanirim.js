    ...
    >
      ...
      >
        <div className="container max-w-5xl mx-auto px-6 lg:px-8">
            ...
            {steps.map((step, index) => (
              ...
              >
                <div className="flex items-center mb-2">
                  <motion.div 
                    className={`p-2 rounded-full mr-3 ${activeStep === step.id ? 'bg-white/20' : 'bg-blue-100 group-hover:bg-blue-200'}`}
          ...
          <AnimatePresence mode='wait'>
            <motion.div
              key={activeStep}
              className="bg-slate-50 p-6 sm:p-8 rounded-xl shadow-lg border border-slate-100 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
            ...
            >
              ...
              <motion.div 
                className="aspect-video bg-slate-200 rounded-lg flex items-center justify-center shadow-inner overflow-hidden relative group"
                initial={{ scale: 0.95, opacity: 0}}
                animate={{ scale: 1, opacity: 1}}
                transition={{ delay: 0.1, duration: 0.3}}
              >
                {/* Replace with actual Loom embed or next/image */}
                <img src={currentStepData.image || 'https://via.placeholder.com/1280x720.png?text=Adım+Görseli'} alt={`${currentStepData.name} görseli`} className="w-full h-full object-cover"/>
      ...
      >
        <div className="container max-w-4xl mx-auto px-6 lg:px-8 text-center">
          ...
          <Link href="/docs" legacyBehavior> {/* Assuming a /docs page will exist */}
