...
export async function fetchVeeqoOrders_adapted(userId, veeqoApiKey, prisma) {
  ...
  try {
    ...
    for (const vOrder of veeqoOrders) {
      ...
      // Map order fields to Prisma schema
      const orderDataToUpsert = {
        ...
        notes: typeof get(vOrder, 'notes') === 'string' ? get(vOrder, 'notes', '').trim() : '',
        orderNumber: vOrder.order_number || vOrder.number || null,
        rawData: vOrder,
      };
      ...
      if (Array.isArray(vOrder.line_items)) {
        for (const vItem of vOrder.line_items) {
          lineItemsData.push({
            sku: get(vItem, 'sku') || 'UNKNOWN',
            productName: get(vItem, 'title') || get(vItem, 'sellable.name') || get(vItem, 'product.title') || 'Unknown Product',
            variantInfo: get(vItem, 'sellable.variant_options_string') || null,
            quantity: parseInt(get(vItem, 'quantity', 0), 10),
            unitPrice: parseFloat(get(vItem, 'price_before_discount_including_tax', 0)),
            totalPrice: parseFloat(get(vItem, 'price_before_discount_including_tax', 0)) * parseInt(get(vItem, 'quantity', 0), 10),
            image: get(vItem, 'image_url') || get(vItem, 'sellable.image_url') || get(vItem, 'sellable.images.0.url') || null,
            notes: typeof get(vItem, 'additional_options') === 'string' ? get(vItem, 'additional_options', '').trim() : '',
            marketplaceKey: String(get(vItem, 'id') || ''),
            orderNumber: vOrder.order_number || vOrder.number || null,
            uniqueLineKey: get(vItem, 'id') ? `${vOrder.id}-${get(vItem, 'id')}` : null,
            remoteLineId: get(vItem, 'id') ? String(get(vItem, 'id')) : null,
          });
        }
      }
