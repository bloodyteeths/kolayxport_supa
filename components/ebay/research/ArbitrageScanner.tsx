...
import {
  ...
  ExternalLink, AlertTriangle, Zap, Settings,
} from 'lucide-react';
    ...
    <Box>
      ...
      <Paper sx={{ p: 2, mb: 2, background: 'linear-gradient(135deg, #1a237e 0%, #4527a0 100%)', color: 'white' }}>
        ...
        <Typography variant="body2" sx={{ opacity: 0.85, mt: 0.5 }}>
          Trendyol kategorilerinden ürünleri çekip eBay fiyatlarıyla karşılaştırın
        </Typography>
      </Paper>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, mb: 2 }}>
          {CATEGORIES.map(cat => (
            <Chip
              ...
              variant={selectedCategories.includes(cat.slug) ? 'filled' : 'outlined'}
              size="small"
              sx={{ fontWeight: selectedCategories.includes(cat.slug) ? 600 : 400 }}
            />
          ))}
        </Box>
        {selectedCategories.length > 0 && (
          <Typography variant="caption" color="primary" sx={{ display: 'block', mb: 1 }}>
            ...
            <Button size="small" onClick={() => setSelectedCategories([])} sx={{ ml: 1, textTransform: 'none', fontSize: '0.7rem' }}>
              Temizle
            </Button>
          </Typography>
        )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: showSettings ? 2 : 0 }}>
          <Button size="small" startIcon={<Settings size={14} />} onClick={() => setShowSettings(!showSettings)} sx={{ textTransform: 'none' }}>
        ...
        <Collapse in={showSettings}>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
            <TextField label="Döviz Kuru (TRY→USD)" type="number" value={exchangeRateOverride} onChange={e => setExchangeRateOverride(e.target.value)} size="small" sx={{ width: 180 }} placeholder="Otomatik" helperText="Boş = güncel kur" inputProps={{ step: 0.001 }} />
            <TextField label="eBay Komisyon Override (%)" type="number" value={feeOverride} onChange={e => setFeeOverride(e.target.value)} size="small" sx={{ width: 200 }} placeholder="Kategoriye göre" helperText="Boş = otomatik" inputProps={{ step: 0.5 }} />
            <TextField label="Maks Ürün Sayısı" type="number" value={maxResults} onChange={e => setMaxResults(Number(e.target.value))} size="small" sx={{ width: 160 }} />
            ...
            <FormControlLabel
              ...
              label={
                <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  <Tooltip title="Below Standard satıcılar ek %5 komisyon öder"><AlertTriangle size={12} color="#e65100" /></Tooltip>
                </Typography>
              }
            />
          </Box>
          <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1, mb: 2, maxHeight: 200, overflow: 'auto' }}>
            <Typography variant="caption" fontWeight={600} sx={{ mb: 1, display: 'block' }}>eBay Kategori Komisyon Oranları:</Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {CATEGORY_FEES.slice(0, 15).map(cat => (
                <Chip key={cat.label} label={`${cat.labelTr}: %${cat.rate}`} size="small" variant="outlined" sx={{ fontSize: '0.7rem' }} />
              ))}
            </Box>
          </Box>
        </Collapse>
        ...
        <Button
          ...
          sx={{ py: 1.5, fontWeight: 700, background: 'linear-gradient(135deg, #1a237e 0%, #4527a0 100%)' }}
        >
          {loading ? 'Taranıyor...' : `Arbitraj Tara (${selectedCategories.length} kategori)`}
        </Button>
        ...
        {loading && (
          <Box sx={{ mt: 2 }}>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block', textAlign: 'center' }}>
              Trendyol ürünleri çekiliyor ve eBay fiyatlarıyla karşılaştırılıyor...
            </Typography>
          </Box>
        )}
      </Paper>
      ...
      {filtered.map((result, idx) => (
        <ArbitrageCard key={`${result.trendyol.id}-${idx}`} result={result} expanded={expandedIdx === idx} onToggle={() => setExpandedIdx(expandedIdx === idx ? null : idx)} isMobile={isMobile} />
      ))}

      {response && filtered.length === 0 && (
        <Alert severity="info" sx={{ mt: 2 }}>
          {filterVerdict !== 'all' ? 'Bu filtreye uygun sonuç yok.' : 'Hiç sonuç bulunamadı. Farklı kategoriler deneyin.'}
        </Alert>
      )}
    </Box>
  );
...
function StatCard({ label, value, icon, color }: { label: string; value: string | number; icon: string; color?: string }) {
  return (
    <Box sx={{ textAlign: 'center', px: 2, py: 1, minWidth: 100 }}>
      <Typography variant="h6" sx={{ fontWeight: 700, color: color || 'text.primary' }}>{icon} {value}</Typography>
      <Typography variant="caption" color="text.secondary">{label}</Typography>
    </Box>
  );
}

function ArbitrageCard({ result, expanded, onToggle, isMobile }: { result: ArbitrageResult; expanded: boolean; onToggle: () => void; isMobile: boolean }) {
  const { trendyol: tp, ebay, financials: fin, verdict, score } = result;
  const vc = VERDICT_CONFIG[verdict];

  return (
    <Paper sx={{ mb: 1.5, border: `2px solid ${vc.color}20`, overflow: 'hidden', '&:hover': { borderColor: `${vc.color}40` }, transition: 'border-color 0.2s' }}>
      <Box onClick={onToggle} sx={{ p: 1.5, cursor: 'pointer', display: 'flex', gap: 1.5, alignItems: 'center', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
        {tp.imageUrl && <Box component="img" src={tp.imageUrl} alt={tp.name} sx={{ width: 56, height: 56, borderRadius: 1, objectFit: 'cover', flexShrink: 0 }} />}
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="body2" fontWeight={600} noWrap>{tp.name}</Typography>
          <Typography variant="caption" color="text.secondary">{tp.brand} · {tp.categoryName}</Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: isMobile ? 1 : 2, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body2" fontWeight={700} color={fin.profitUsd > 0 ? 'success.main' : 'error.main'}>{fmt(fin.profitUsd)}</Typography>
          </Box>
          <Chip label={`${vc.emoji} ${score}`} size="small" sx={{ bgcolor: vc.bg, color: vc.color, fontWeight: 700 }} />
      ...
      <Collapse in={expanded}>
        <Divider />
        <Box sx={{ p: 2 }}>
          <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>💰 Finansal Detay</Typography>
          <Box sx={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1fr', gap: 1, mb: 2 }}>
            ...
            {fin.internationalFeeUsd > 0 && <FinRow label="Uluslararası" value={fmt(fin.internationalFeeUsd)} sub="1.65%" />}
          ...
          <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>📊 eBay Fiyat Aralığı</Typography>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
          ...
          {ebay.topItems.length > 0 && (
            <>
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>🛒 eBay Benzer Ürünler</Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                {ebay.topItems.slice(0, 3).map((item, i) => (
                  <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'center', p: 1, bgcolor: 'grey.50', borderRadius: 1 }}>
                    {item.imageUrl && <Box component="img" src={item.imageUrl} alt="" sx={{ width: 40, height: 40, borderRadius: 0.5, objectFit: 'cover' }} />}
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        {item.soldQuantity > 0 && <Typography variant="caption" color="success.main">{item.soldQuantity} satış</Typography>}
                      </Box>
                    </Box>
                  </Box>
                ))}
          ...
          <Box sx={{ display: 'flex', gap: 1, mt: 2, flexWrap: 'wrap' }}>
            {tp.url && <Button size="small" variant="outlined" startIcon={<ExternalLink size={14} />} href={tp.url} target="_blank" sx={{ textTransform: 'none' }}>Trendyol</Button>}
            <Button size="small" variant="outlined" startIcon={<ExternalLink size={14} />} href={`https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(tp.name.split(' ').slice(0, 4).join(' '))}`} target="_blank" sx={{ textTransform: 'none' }}>eBay&apos;de Ara</Button>
          </Box>
        </Box>
      </Collapse>
    </Paper>
...
function FinRow({ label, value, sub, bold, color }: { label: string; value: string; sub?: string; bold?: boolean; color?: string }) {
  return (
    <Box sx={{ p: 1, bgcolor: bold ? 'grey.100' : 'transparent', borderRadius: 0.5 }}>
      <Typography variant="caption" color="text.secondary">{label}</Typography>
      <Typography variant="body2" fontWeight={bold ? 700 : 500} sx={{ color: color || 'text.primary' }}>{value}</Typography>
